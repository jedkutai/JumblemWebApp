...
export default function BotGuestPlayCasualGameView({ passedUser, passedGame }: BotGuestPlayCasualGameViewProps) {
    const {
        moves,
...
useEffect(() => {
    ...
    async function botMove() {
        if (wordCheckComplete) {
            if (!gameOver) {
                if (!yourTurn) {
                    const moveDelay = movesCopy.length < 6 ? Math.floor(Math.random() * 3) + 1 : Math.floor(Math.random() * 5) + 2;
                    const timeout = setTimeout(async () => {
                        try {
