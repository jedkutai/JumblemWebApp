...
export class GameService {
  static async updateLetterBank(game: GameModel, playerOne: boolean, letterBank: string[]): Promise<void> {
    const db = getFirestore();

    if (playerOne) {
      await updateDoc(doc(db, "newGames", game.id), { playerOneLetterBank: letterBank });
    } else {
      await updateDoc(doc(db, "newGames", game.id), { playerTwoLetterBank: letterBank });
    }
